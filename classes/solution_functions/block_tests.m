...
for is1 = 1: (size(lath_solutions.array,2)-1)
    for is2 = (is1+1): size(lath_solutions.array,2)
        ...
        % plastic slip
%         if sum(sum(abs(F1 - F2 ))) < delta_F_min
%             neg_diff = neg_diff + 1;
%             continue
%         end
...
disp( ['Second crit: ', num2str(neg_lamda2), ' neglected because lamda2 of block deviates more than ', num2str(lambda2_tol), ' from 1'] );
disp( ['Third crit: ', num2str(neg_diff), ' neglected because Fs do no differ in the d1 norm more than ', num2str(delta_F_min) ] ); 
