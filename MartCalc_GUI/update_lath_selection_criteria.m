    ...
    case 1 % IPS solutions
        ...
        if ~isempty(handles.martensite.IPS_solutions.array) %solutions_available
            ...
            if handles.asc_number_IPS > 0
                ...
                % note 557 is 9.4째 from 111 ! therefore this high tolerance!
                % Angle between 111 and 557 habit plane
                % acos( dot([1. 1. 1.], [5. 5. 7.])/(sqrt(3)*sqrt(99) ) ) = 9.4 degree
                if(handles.asc_status_IPS(4) > 0)
                    theta_h_to_cpp = str2num(handles.pan_asc_IPS.Children(size(handles.pan_asc_IPS.Children,1)+1-handles.asc_status_IPS(4)).Children(2).String);
                end
                ...
                if(handles.asc_status_IPS(5) > 0)
                    theta_KS_max = str2num(handles.pan_asc_IPS.Children(size(handles.pan_asc_IPS.Children,1)+1-handles.asc_status_IPS(5)).Children(2).String);
                end
                ...
                if(handles.asc_status_IPS(6) > 0)
                    theta_NW_max = str2num(handles.pan_asc_IPS.Children(size(handles.pan_asc_IPS.Children,1)+1-handles.asc_status_IPS(6)).Children(2).String);
                end
                ...
                if(handles.asc_status_IPS(7) > 0)
                    theta_max_ILSdir_to_h = str2num(handles.pan_asc_IPS.Children(size(handles.pan_asc_IPS.Children,1)+1-handles.asc_status_IPS(7)).Children(2).String);
                end
        ...
        if ~isempty(handles.martensite.ILS_solutions.array)
            ...
            if handles.asc_number_ILS > 0
                ...
                if (handles.asc_status_ILS(1) > 0)
                    min_stepwidth = str2num(handles.pan_asc_ILS.Children(size(handles.pan_asc_ILS.Children,1)+1-handles.asc_status_ILS(1)).Children(2).String);
                end
