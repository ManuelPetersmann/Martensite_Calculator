function [ ] = delete_ac_MartCalc( hObject,eventdata)
%DELETE_ASC_MARTCALC Summary of this function goes here
%   Detailed explanation goes here

% retrieve data of GUI
handles = guidata(hObject);
button_parent = get(hObject,'Parent'); % get parent panel of ac

main_ac_panel = get(button_parent,'Parent'); % get main panel for ac
...
if (main_ac_panel == handles.pan_asc) % panel for active selection criteria for calculation of laths
        ...
        while(postion_in_asc_list <= size(handles.pan_asc.Children,1))
           ...
           handles.pan_asc.Children(size(handles.pan_asc.Children,1)+1-postion_in_asc_list).Position(2)=...
                handles.pan_asc.Children(size(handles.pan_asc.Children,1)+1-postion_in_asc_list).Position(2)+((handles.pan_asc.Position(4)/handles.asc_maxcrit)/handles.pan_asc.Position(4));
%                 handles.pan_asc.Children(size(handles.pan_asc.Children,1)+1-postion_in_asc_list).Position(2)+((handles.pan_asc.Position(4)/8)/handles.pan_asc.Position(4));
    ...
    elseif (main_ac_panel == handles.pan_amc) % panel for mixing of lath solutions to blocks
        postion_in_amc_list = handles.amc_status(criterion_number);
        % delete actual asc panel
        % NOTE: with deletion of this panel, the following panels in the list are
        % switched one position forward in the list of .Children!
        % % % handles.pan_asc.Children(handles.asc_number-postion_in_asc_list+1).delete(); 
        handles.pan_amc.Children(size(handles.pan_amc.Children,1)+1-postion_in_amc_list).delete() 
        handles.amc_number = handles.amc_number - 1; % decrease number of asc
        handles.amc_status(criterion_number) = 0; % set criterion inactive
        
        % update positions of the following list entries
        % % while(handles.asc_list(postion_in_asc_list) > 0)
        while(postion_in_amc_list <= size(handles.pan_amc.Children,1))
           % update position of the panel
           handles.pan_amc.Children(size(handles.pan_amc.Children,1)+1-postion_in_amc_list).Position(2)=...
                handles.pan_amc.Children(size(handles.pan_amc.Children,1)+1-postion_in_amc_list).Position(2)+((handles.pan_amc.Position(4)/handles.amc_maxcrit)/handles.pan_amc.Position(4));
%                 handles.pan_amc.Children(size(handles.pan_amc.Children,1)+1-postion_in_amc_list).Position(2)+((handles.pan_amc.Position(4)/8)/handles.pan_amc.Position(4));

           % update number of position in list
           handles.pan_amc.Children(size(handles.pan_amc.Children,1)+1-postion_in_amc_list).Children(4).String = num2str(postion_in_amc_list);

           % update status of criterion in order to fit new position in list
           handles.amc_status(handles.pan_amc.Children(size(handles.pan_amc.Children,1)+1-postion_in_amc_list).Children(3).Value) = postion_in_amc_list; 

           handles.amc_list(postion_in_amc_list)=handles.pan_amc.Children(postion_in_amc_list).Children(3).Value;

           postion_in_amc_list = postion_in_amc_list+1;
        end
