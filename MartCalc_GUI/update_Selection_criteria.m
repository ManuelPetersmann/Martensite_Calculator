...
if handles.asc_number > 0
    ...
    for criterion = 1:handles.asc_number
        ...
        %
        disp('see update_selection_criteria!!!')
        ...
        switch handles.asc_list( criterion )
            case 1 % stepwidth
                ...
                updateLog_MartCalc(hObject, handles, ['Solutions reduced to : ', num2str(length(red_sols.array)) ' for a stepwidth > ',num2str(stepwidth)] );
            case 2 % eps_ips
                red_sols = Solution_array( Slip_solution(), red_sols, 'eps_ips', eps_ips_max, 'max' );
                updateLog_MartCalc(hObject, handles, ['Solutions reduced to : ', num2str(length(red_sols.array)) ' for a shape strain  < ',num2str(eps_ips_max)] );
            case 3 % theta_CPPs
                red_sols = Solution_array( Slip_solution(), red_sols, handles.austenite.CPPs, theta_CPPs_max, 'theta_CPPs', 'closest_to_cpp', 'cpps_gamma', true);
                updateLog_MartCalc(hObject, handles, ['Solutions reduced to : ', num2str(length(red_sols.array)) ' for misorietation of CPPs  < ',num2str(theta_CPPs_max),'°'] );
            case 4 % theta_h_to_cpp
                red_sols = Solution_array( Slip_solution(), red_sols, handles.austenite.CPPs, theta_h_to_cpp, 'theta_h_to_cpp', 'closest_to_h', 'h');
                updateLog_MartCalc(hObject, handles, ['Solutions reduced to : ', num2str(length(red_sols.array)) ' for habit plane misorientation to CP-planes  < ',num2str(theta_h_to_cpp),'°'] );
            case 5
                red_sols = Solution_array( Slip_solution(), red_sols, 'delta_determinant_max', delta_determinant_max,  det(handles.martensite.U));
                updateLog_MartCalc(hObject, handles, ['Solutions reduced to : ', num2str(length(red_sols.array)) ' for (non-physical) volume change  < ',num2str(delta_determinant_max)] );
            case 6
                red_sols = Solution_array( Slip_solution(), red_sols, handles.austenite.CP_dirs, theta_KS_max, 'theta_KS_min', 'closest_cp_direction', 'KS', false );
                updateLog_MartCalc(hObject, handles, ['Solutions reduced to : ', num2str(length(red_sols.array)) ' for a maximum deviation angle of KS-directions  < ',num2str(theta_KS_max),'°'] );
            case 7
                red_sols = Solution_array( Slip_solution(), red_sols, handles.NW, theta_NW_max, 'theta_NW_min', 'closest_NW', 'NW', false);
                updateLog_MartCalc(hObject, handles, ['Solutions reduced to : ', num2str(length(red_sols.array)) ' for a maximum deviation angle of NW-directions  < ',num2str(theta_NW_max),'°'] );
            case 8 % theta_max_ILSdir_to_h
