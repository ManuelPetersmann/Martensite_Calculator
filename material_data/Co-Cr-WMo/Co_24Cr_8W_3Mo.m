...
%%
%orientations_per_grain = 4

% alpha = [ 189.3, 186.8, 285.4, 187.2, 271.0 ];
% beta =  [ 85.5,  88.7,  48.0,  84.4,  138.1 ];
% gamma = [ 311.6, 312.2, 82.4,  312.3, 348.8 ];

% checking two sides of an interface
% side one 
% alpha = [322.3, 357.3, 351.4, 237.8 ];
% beta =  [114.9, 27.8,  29.0 , 77.2  ];
% gamma = [254.8, 58.7,  153.5, 335.2 ];

% figure;
% plot_polefigure([1 1 1], [alpha(1), beta(1), gamma(1)], 'bungee')
% figure;
% plot_polefigure([1 1 1], [alpha(2), beta(2), gamma(2)], 'bungee')

% side two
% alpha = [ 237.6, 92.8,  276.4, 92.1 ];
% beta =  [ 141.8, 38.8,  140.8, 37.8 ];
% gamma = [ 352.4, 278.1, 354.2, 279.4];
...
a_mart = 2.534; % Angström
c_mart = 4.097;
%martensite.Lp = a_mart *[1 1 1];  % 2.8807346   

%% compare lattice parameters of pure cobalt - from Khachaturyans book p.19 
a_au_cob  = 3.552;
a_mar_cob = 2.507;
c_mar_cob = 4.069;
% results in 
...
% U1 = [ 	1.0928	0.0262	0.0904;
%        	0.0262	0.8362	0.0262;
%        	0.0904	0.0262	1.0928 	]
%%
...
U1 = [1.0947     -0.0923     -0.0239;
     -0.0923      1.0947      0.0239;
     -0.0239      0.0239      0.8390];
martensite.U = U1;
...
% U2 = [0.8913    -0.1551     -0.1112;
%      -0.1551     1.3370     -0.1551;
%      -0.1112    -0.1551      0.8913];
...
%       0.        0.7088      0.;
%       0.        0.          1.2277];
% display(['Volume change of U3 in percent is:', num2str( ( det(U3)-1)*100 ) ] );  
%%
