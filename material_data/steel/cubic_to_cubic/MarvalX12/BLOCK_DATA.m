...
% Bainstrain, was nicht erklärbar ist (ws grosse Rundungsfehler) 
...
%% my calculations with martensite calculator

% using this yields different 24 blocks
% block = [1.0499   -0.0700   -0.0220
%         -0.0630    1.1186   -0.0006
%          0.0381   -0.0220    0.8738];

% using this yields 12 different blocks
block = [ 1.1243     0.0541         0.
           0.0371    1.0218         0.
           0.        0.          0.8923 ]
...
all_operators = matrixArray_reduzieren(all_operators, 1e-4)

blockpairs = zeros(12,2);
count = 1;
for i = 1:size(VariantenSort,3)
    for j = i:size(VariantenSort,3)
...
fileID = fopen('block2_data.txt', 'a');
...
%    strainSort(:,:,blockpairs(i,2)) = 0.5*( VariantenSort(:,:,blockpairs(i,2))' + VariantenSort(:,:,blockpairs(i,2)) ) - eye(3) ;
    strainSort(:,:,i) = El_from_F( VariantenSort(:,:,i) ) ;
...
%    fprintf(fileID,format_block_variant, get_order( strainSort(:,:,blockpairs(i,2)) ) );
    fprintf(fileID,format_block_variant, get_order( strainSort(:,:,i) ) );
end
