...
g_min_sols = Solution_array( Slip_solution(), all_sols_specific_slipsys, 'g', beta_min, 'min'); 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% reduce solutions to ones with eps < something 
%eps_max = 0.5; % 0.3; % 0.3; % 100.; 
% eps_max_solutions = Solution_array( Slip_solution(), g_min_sols, 'eps', eps_max, 'max' ); 
eps_max_solutions = Solution_array( Slip_solution(), all_sols_MarescaCurtin, 'eps', eps_max, 'max' ); % for MARESCA&CURTIN RUN
...
mc_sols = det_sols.sort( 'theta_p' ) % sort in ascending order for specific property
